...
v1.5.1: adjusts the guidance wording for current Claude models, which follow instructions literally. Step one of the guidance for government official documents now tells the model to infer what it can from the request and to ask only when a detail cannot be inferred and would change the document; two content errors in that guidance were also fixed. These changes also reach the full single-file edition and the Codex and Gemini entry files. Separately, one duplicated emphasis line was removed from `SKILL.md` (the same line stays in the single-file edition for now), and the LITE edition corrects a meeting-record field label. None of this has been tested against models on any platform. See the [CHANGELOG](CHANGELOG.md).
...
### Claude Code (including the desktop app)

**Method A (recommended): install the plugin from the marketplace**

This repo is itself a Claude Code plugin marketplace. In Claude Code (CLI or desktop app), run:
...
Use `/plugin` afterwards to enable or disable it. To update to a newer version, run `claude plugin update tw-formal-writing` in your terminal, then restart Claude Code to apply.
