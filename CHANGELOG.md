...
盤點 skill 內給模型的指示文字，對照目前 Claude 模型照字面遵循指示的特性，改寫要求逐項詢問的確認寫法，刪去重複的強調句與檢核步驟，並修正三處內容錯誤。檢核項目沒有減少。這些調整未做模型實測；單檔版（`STANDALONE.md`、`AGENTS.md`、`GEMINI.md`）與 plugin 複本同步改動。
...
- **品質檢核關卡刪去重複的「檢核流程」**：四步驟流程與上方各檢核表內容重複，刪除；原本只寫在流程裡的分項標號層次，移入改名後的「格式檢核」表。「有問題立即修正後再交付」的要求仍在關卡開頭。
- **`SKILL.md` 刪去重複的強調句**：「讀取 reference file 後，嚴格依照該 file 的指示撰寫」與上方「讀取對應的 reference file 並依其規範撰寫」重複。
- 範例四與維護說明（`CLAUDE.md`）刪去版本沿革註記，只留現行規則與官方出處。

### Fixed
- **「幫我改」錯誤樣態名稱錯置**：原寫「主旨使用准駁語」，所舉例子卻是「請查照」在說明段重複出現，屬期望語位置問題。改為「期望語位置錯誤」，與品質檢核表的用語一致。
