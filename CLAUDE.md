...
- 公文用語的**行文方向正確性**（稱謂語鈞 / 貴 / 大、期望語等）：這是這個 skill 的核心價值，改動前確認符合《文書處理手冊》。

## 規範定位與修改準則

- 這個 skill 提供通用指引，不是標準答案或唯一解釋，立場與《文書處理手冊》一致。
- 修正既有內容時，只改誤解原文、誤引原文或無中生有的地方；不屬於這三種情形就不動。
- 新增內容只收官方來源（《文書處理手冊》、法規、行政院釋例、國家文官學院教材），並標出處。各機關的內規與慣用寫法予以尊重，但不收進 repo；想貼近自家機關習慣的人可以 fork 客製化。
- 手冊沒有寫明的用法，或沒有把握的地方，不自行改得更嚴格。
- 來源位階：手冊與法規優先；國家文官學院教材可補手冊未規定之處，但不推翻手冊；釋例與教材說法不同時兩邊並列，不自行裁決（見 `CITATIONS.md`「教學來源使用紀律」）。
