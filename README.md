...
v1.5.1：依目前 Claude 模型照字面遵循指示的特性調整規範寫法。公文第一步的資訊確認改為能從描述推知的直接判斷，推知不了且會影響寫法才詢問；另刪去一句重複的強調句，並修正三處內容錯誤。完整單檔版與 Codex、Gemini 的入口檔同步改動，各平台都未做模型實測，詳見 [CHANGELOG](CHANGELOG.md)。
