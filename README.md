...
v1.5.1：依目前 Claude 模型照字面遵循指示的特性調整規範寫法。公文第一步的資訊確認改為能從描述推知的直接判斷，推知不了且會影響寫法才詢問，並修正公文規範兩處內容錯誤；這些公文規範的改動同步進入完整單檔版與 Codex、Gemini 的入口檔。另刪去 `SKILL.md` 一行重複的強調說明（單檔版的同一行暫不刪），LITE 版修正會議紀錄欄位用字。各平台都未做模型實測，詳見 [CHANGELOG](CHANGELOG.md)。
